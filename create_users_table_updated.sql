...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'website_db') THEN
        CREATE DATABASE website_db;
    END IF;
END
$$;

-- Переключаемся на базу данных website_db (выполнить в DBeaver)
-- Далее выполнять в базе данных website_db
...
-- Удаляем существующих пользователей перед добавлением новых
